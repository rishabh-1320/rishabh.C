...
import { useMarqueeTooltipVisible } from "./marquee";
...
export function AiToolTile({ name, icon }: { name: string; icon: string }) {
  ...
  return (
    <div
      className="relative flex size-20 shrink-0 items-center justify-center rounded-ds-card border border-ds-hairline bg-ds-surface-paper md:size-24"
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={icon} alt={name} className="h-8 w-8 object-contain md:h-10 md:w-10" />
